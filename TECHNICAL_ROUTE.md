...
| M4 | Pairing + policy: nonce SAS, trust store, auto-accept rules, block list, rate limiting, collision naming, filename sanitizer | **DONE** | M | Verified via the CLI harness: unknown sender → both sides show matching codes → paired; with `receive/policy = auto-accept-paired` a transfer from a paired peer is accepted with **zero prompts**; `lanpipe block` from a *separate process* makes the running server reject with 403 and no prompt, `unblock` restores it; a blocked peer cannot pair either; a peer whose fingerprint changed is rejected; the hostile-filename vector table (traversal, Windows reserved names, trailing dot/space, control and bidi characters, non-portable characters, overlong) is in `tests/tst_sanitizer.cpp`; two concurrent `prepare`s → 409. Prompt rate limiting (3 per minute per device) is covered by `tst_receiveservice`. The approval *dialog* itself is M5 |
