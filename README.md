...
| 接收策略：一律提示 / 已配对自动接受 / 开放模式 | 完成 |
| 黑名单与提示限流（按 deviceId，每分钟 3 次） | 完成，`lanpipe block` / `unblock` / `devices` |
| 图形界面 | 未开始（M5） |
...
接收方会列出**是谁、几个文件、多大**并要求确认（`y` 接受 / `n` 拒绝 / `b` 拒绝并屏蔽）；
传输期间每 10% 打一行进度。接收方按 Ctrl-C 可以取消当前传输。

设备和屏蔽关系：

```bash
lanpipe devices                # 已配对与已屏蔽的列表
lanpipe block bc28f694         # 屏蔽（前缀即可）：它之后连审批框都弹不出来
lanpipe unblock bc28f694       # 解除屏蔽，配对关系不受影响
```

屏蔽对**正在跑的 `serve` 立刻生效**（接收方每次 prepare 前重读信任库），
不需要重启。默认策略是一律提示；想让它对已配对设备静默接受，在接收方的
`lanpipe.conf` 里写 `[receive] policy=auto-accept-paired`。
